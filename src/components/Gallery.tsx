...
export default function Gallery() {
  ...
  const showNext = (e?: React.MouseEvent | React.TouchEvent | KeyboardEvent) => {
    e?.stopPropagation();
    setCurrentIndex((prev) => (prev === galleryImages.length - 1 ? 0 : prev + 1));
  };

  const showPrev = (e?: React.MouseEvent | React.TouchEvent | KeyboardEvent) => {
    e?.stopPropagation();
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {galleryImages.map((img, index) => (
            <button
              ...
              className="relative group overflow-hidden rounded-xl cursor-pointer aspect-[4/3] bg-gray-200 w-full p-0 text-left focus-visible:ring-4 focus-visible:ring-safetyOrange focus-visible:ring-offset-2 focus:outline-none"
