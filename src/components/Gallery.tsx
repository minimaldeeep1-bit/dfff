...
export default function Gallery() {
  ...
  const showNext = useCallback((e?: React.MouseEvent | React.TouchEvent | KeyboardEvent) => {
    e?.stopPropagation();
    setCurrentIndex((prev) => (prev === galleryImages.length - 1 ? 0 : prev + 1));
  }, []);

  const showPrev = useCallback((e?: React.MouseEvent | React.TouchEvent | KeyboardEvent) => {
    e?.stopPropagation();
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {galleryImages.map((img, index) => (
            <button
              ...
              className="w-full relative group overflow-hidden rounded-xl cursor-pointer aspect-[4/3] bg-gray-200 focus:outline-none focus-visible:ring-4 focus-visible:ring-safetyOrange"
            >
              ...
              <div className="absolute inset-0 bg-slateGray/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col items-center justify-center">
                <Maximize2 aria-hidden="true" className="text-white w-8 h-8 mb-2" />
